...
const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      if (avgFps < 50) next = clampVal(currentDprRef.current * 0.85, dprFloor, base);
      else if (avgFps > 58 && currentDprRef.current < base) next = clampVal(currentDprRef.current * 1.1, dprFloor, base);
